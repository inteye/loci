...
impl Tool for FileRead {
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<Value> {
        let path = params["path"].as_str().ok_or_else(|| AppError::Tool("missing path".into()))?;
...
impl Tool for FileWrite {
    ...
    fn description(&self) -> &str { "Write content to a file. Creates the file if it doesn't exist." }
    fn parameters_schema(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
