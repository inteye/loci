...
impl MemoryStore {
    ...
    pub async fn save(&self, memory: &Memory) -> Result<()> {
        let embedding_blob: Option<Vec<u8>> = memory.embedding.as_ref().map(|v| {
    ...
    pub async fn recall(&self, query_embedding: Option<&[f32]>, scope: Option<MemoryScope>, project_id: Option<Uuid>, limit: usize) -> Result<Vec<Memory>> {
        let scope_filter = scope.as_ref().map(|s| serde_json::to_string(s).unwrap_or_default());
        ...
        .bind(&scope_filter).bind(&scope_filter)
        .bind(&project_filter).bind(&project_filter)
        .bind(Utc::now().to_rfc3339())
        .fetch_all(&self.pool).await?;

        let mut memories: Vec<(Memory, Option<Vec<f32>>)> = rows.into_iter().filter_map(|row| {
            ...
            let mem = Memory {
                ...
                expires_at: row.get::<Option<&str>, _>("expires_at").and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok()).map(|d| d.with_timezone(&Utc)),
            };
            Some((mem, embedding))
        }).collect();
...
pub async fn remember(store: &MemoryStore, content: &str, scope: MemoryScope, project_id: Option<Uuid>, embedding: Option<Vec<f32>>) -> Result<Memory> {
    let mem = Memory {
        id: Uuid::new_v4(),
        scope,
