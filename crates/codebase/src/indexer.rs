use crate::{
    rust_parser::{ParsedFile, RustParser},
    scanner::{Language, ProjectScanner, ProjectSummary},
    ts_parser::TsParser,
};
use anyhow::Result;
use std::path::Path;
...
impl CodebaseIndexer {
    pub fn index(root: &Path) -> Result<CodebaseIndex> {
        ...
        let parsed_files: Vec<ParsedFile> = summary
            .files
            .iter()
            .filter_map(|f| match f.language {
                ...
                | Language::JavaScript
                | Language::Go
                | Language::Java => TsParser::parse_file(&f.path).ok(),
                _ => None,
            })
            .collect();
    ...
    pub fn index_incremental(
        ...
        for file in &summary.files {
            ...
            let parsed = match file.language {
                ...
                | Language::JavaScript
                | Language::Go
                | Language::Java => TsParser::parse_file(&file.path).ok(),
                _ => None,
            };

            if let Some(pf) = parsed {
                // Replace or insert
                if let Some(existing_pf) =
                    existing.parsed_files.iter_mut().find(|p| p.path == pf.path)
                {
                    *existing_pf = pf;
                } else {
