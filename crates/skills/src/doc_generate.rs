...
impl Skill for PrDescriptionSkill {
    ...
    fn description(&self) -> &str {
        "Generate a PR description from a git diff or commit list"
    }
...
async fn build_doc_context(input: &str, llm: &dyn LlmClient, ctx: &ToolContext) -> Result<String> {
    ...
    if let Some(qv) = query_vec.as_deref() {
        if let Ok(hits) = vector_index.search(qv, 24).await {
            ids.extend(hits.into_iter().filter_map(|(id, _)| {
                graph
                    .nodes
                    .iter()
                    .find(|n| {
                        n.id == id && matches!(n.kind, NodeKind::Decision | NodeKind::Concept)
                    })
                    .map(|n| n.id)
            }));
        }
            ...
            graph
                ...
                .filter(|n| {
                    lowered.contains(&n.name.to_lowercase())
                        || n.description
                            .as_deref()
                            .map(|d| lowered.contains(&d.to_lowercase()))
                            .unwrap_or(false)
                })
                .map(|n| n.id)
                .take(8),
    ...
    let decisions: Vec<String> = ids
        .iter()
        .filter_map(|id| {
            graph
                .nodes
                .iter()
                .find(|n| n.id == *id && n.kind == NodeKind::Decision)
        })
        .map(|n| format!("- {}: {}", n.name, n.description.as_deref().unwrap_or("")))
        .collect();

    let concepts: Vec<String> = ids
        .iter()
        .filter_map(|id| {
            graph
                .nodes
                .iter()
                .find(|n| n.id == *id && n.kind == NodeKind::Concept)
        })
        .map(|n| format!("- {}: {}", n.name, n.description.as_deref().unwrap_or("")))
        .collect();
