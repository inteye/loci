...
fn api_err<T>(code: &'static str, message: impl Into<String>) -> ApiResult<T> {
    let status = match code {
        "empty_question" | "empty_target" | "empty_query" | "invalid_kind" | "invalid_path" => {
...
async fn handle_project_use(
    ...
    if let Some(project) = registry
        .projects
        .iter()
        .find(|project| project.name == req.name)
        .cloned()
    {
...
async fn handle_knowledge_search(
    ...
    let items = kb
        .search_external(query_embedding.as_deref(), Some(&req.query), 10)
        .await
        .unwrap_or_default()
        .into_iter()
        .map(|item| KnowledgeItemResponse {
...
async fn handle_knowledge_add(
    ...
    let kb = match KnowledgeStore::new(&path.join(".bs/knowledge.db").to_string_lossy()).await {
        Ok(store) => store,
        Err(_) => {
            return Json(KnowledgeAddResponse {
                source: req.source,
                chars: 0,
            });
        }
    ...
    let knowledge = if source.starts_with("http://") || source.starts_with("https://") {
        ...
        let embedding =
            if let (Some(ref llm), Some(content)) = (llm.as_ref(), file_content.as_ref()) {
        ...
        return match history {
            Ok(history) => Json(HistoryResponse {
                ...
                commits: history
                    .commits
                    .into_iter()
                    .map(|commit| HistoryCommitResponse {
                        hash: commit.hash,
...
async fn answer_question(state: &AppState, question: &str, provider: Option<&str>) -> String {
    ...
    match cfg.build_client(provider) {
        ...
        Ok(llm) => {
            ...
            let store = match GraphStore::new(&db).await {
                Ok(s) => s,
...
fn build_doc_prompt(graph: &loci_graph::KnowledgeGraph, kind: &str) -> String {
    let decisions: Vec<&loci_graph::Node> = graph
        .nodes
        .iter()
        .filter(|n| n.kind == NodeKind::Decision)
        .take(12)
        .collect();
    let concepts: Vec<&loci_graph::Node> = graph
        .nodes
        .iter()
        .filter(|n| n.kind == NodeKind::Concept)
        .take(12)
        .collect();
    let files: Vec<&loci_graph::Node> = graph
        .nodes
        .iter()
...
async fn score_eval_answer(
    ...
    match llm
        ...
        .await
    {
        Ok(loci_llm::LlmResponse::Text(text)) => serde_json::from_str::<EvalScore>(&text)
...
async fn load_trace_response(project_path: &str, target: Option<&str>) -> TraceResponse {
    ...
    match GraphStore::new(&db.to_string_lossy()).await {
        Ok(store) => {
            let graph = store.load_graph().await.unwrap_or_default();
            let anchor_ids: HashSet<uuid::Uuid> = graph
                .nodes
                .iter()
                .filter(|node| {
                    if query.is_empty() {
...
async fn handle_memories(State(state): State<AppState>) -> Json<MemoryResponse> {
    ...
    match MemoryStore::new(&db).await {
        Ok(store) => {
            ...
            Json(MemoryResponse {
                memories: mems.into_iter().map(|m| m.content).collect(),
            })
        }
...
async fn handle_project_add_v1(
    ...
    if registry
        .projects
        .iter()
        .any(|project| project.name == req.name)
    {
        return api_err(
            "project_already_exists",
            format!("Project '{}' already exists", req.name),
        );
    }
    let abs = std::fs::canonicalize(&req.path)
        .unwrap_or_else(|_| std::path::PathBuf::from(&req.path))
...
async fn handle_project_use_v1(
    ...
    api_err(
        "project_not_found",
        format!("Project '{}' not found", req.name),
    )
}
...
async fn handle_knowledge_list_v1(
    State(state): State<AppState>,
...
async fn handle_knowledge_add_v1(
    ...
    if response.chars == 0 {
        return api_err(
            "knowledge_ingest_failed",
            format!("Failed to ingest '{}'", response.source),
        );
    }
    api_ok(response)
}
...
async fn handle_eval_v1(
    ...
    if response.results.is_empty()
        && response
            .drift_check
            .iter()
            .any(|line| line == "LLM not configured.")
    {
...
async fn main() -> anyhow::Result<()> {
    ...
    let port: u16 = std::env::var("BS_PORT")
        .ok()
